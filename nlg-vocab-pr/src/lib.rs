//! Pull-request narrative vocabulary for the `nlg` engine.
//!
//! Registers higher-level PR narrative templates that complement the raw event
//! coverage in `nlg-vocab-git`. Where `nlg-vocab-git` covers discrete events
...
fn register_review_state(engine: &mut Engine) -> Result<(), NlgError> {
...
fn register_scope(engine: &mut Engine) -> Result<(), NlgError> {
...
fn register_diff_stats(engine: &mut Engine) -> Result<(), NlgError> {
...
fn register_age(engine: &mut Engine) -> Result<(), NlgError> {
    // Low: bare age.
    engine.register_template_at(
        "pr.age",
...
fn register_merge_readiness(engine: &mut Engine) -> Result<(), NlgError> {
...
fn register_ci_status(engine: &mut Engine) -> Result<(), NlgError> {
...
fn register_related_prs(engine: &mut Engine) -> Result<(), NlgError> {
