//! Bridge `tracing` events to natural language prose via the nlg engine.
//!
//! `nlg-tracing` provides [`NlgLayer`], a [`tracing_subscriber::Layer`] that
//! intercepts tracing events, converts their fields into an [`nlg_core::Context`],
//! and renders prose via a registered [`nlg_core::Engine`].
//!
//! # Example
//!
//! ```rust
...
/// Visits tracing event fields and populates an [`nlg_core::Context`].
...
mod tests {
    use super::*;
    use nlg_core::{Strictness, Variation};
    use nlg_grammar_en::English;
